...
function HeroSection() {
	...
	// Fetch real statistics for dynamic content
	useEffect(() => {
		const fetchStats = async () => {
			try {
				...
				let totalDesigns = 50000; // Default fallback
				let totalVendors = 5000; // Default fallback

				if (productsResponse.status === 'fulfilled') {
					totalDesigns = productsResponse.value.data.total || totalDesigns;
				}

				if (vendorsResponse.status === 'fulfilled') {
					totalVendors = vendorsResponse.value.data.total || totalVendors;
				}
				...
				<div className="max-w-2xl">
					...
					{!stats.loading && (
						<div className="flex flex-wrap gap-6 mb-8 text-white/90">
							...
							<div className="text-center">
								...
								<div className="text-sm">Expert Designers</div>
							</div>
							<div className="text-center">
					...
					<div className="flex flex-col sm:flex-row gap-4">
						<Button
							...
							{isAuthenticated ? 'Continue Designing' : 'Start Designing'}
					...
					{isAuthenticated && (
						<div className="mt-6 bg-white/10 backdrop-blur-sm rounded-lg p-4">
							<p className="text-white/90 text-sm">
								...
								{user?.role === 'vendor' && (
									<span className="ml-2 bg-yellow-400/20 text-yellow-200 px-2 py-1 rounded text-xs">
										Vendor
									</span>
								)}
							</p>
